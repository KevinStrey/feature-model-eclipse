"""
Média de LOC por Método por Feature ao longo das Releases do EclipseIDE.

Gera gráfico de linha do tempo com a média de LOC por método ativo de cada
feature ao longo de todas as versões. Cada feature é uma linha no gráfico.
...
out_dir  = os.path.join(base_dir, r"6-Analises\1-Sistema_completo_features\LOC_metodos")
...
# ── 3. Processar Evolução (simular estado acumulado por feature) ─────────────
print("Processando evolução...")
feat_names = df_all['feature'].unique()
current_state = {feat: {} for feat in feat_names}
feature_metrics = []
...
for r in release_order:
    ...
    for feat in feat_names:
        ...
        mean_loc = float(np.mean(active_locs)) if active_locs else 0.0
...
# Calcular Mean LOC Normalizado (0-100) por feature
...
# (usando LOC total para classificação, consistente com o critério do projeto)
feature_total_loc = {}
for feat in feat_names:
    feat_state = current_state[feat]
    total = sum(v['LOC'] for v in feat_state.values() if v['LOC'] > 0)
    feature_total_loc[feat] = total

# Usar o LOC da primeira release para classificar (consistente com script de referência)
first_release_loc = {}
for feat in feat_names:
    feat_data = df_metrics[df_metrics['feature'] == feat]
    # Calcular LOC total na primeira release via current_state rebuild
    # (aproximação: usar o valor de Mean LOC * nº métodos não serve — recalcular)
    first_release_loc[feat] = feature_total_loc.get(feat, 0)

# Recalcular usando a mesma lógica do script de referência
...
for r in release_order:
    ...
    # Após a primeira release, capturar o LOC inicial
...
def plot_mean_loc(df_sub, features_list, title_suffix, filename):
    """
    Plota Mean LOC per method ao longo das releases.
    """
    n = len(features_list)
    ...
    for i, feat in enumerate(features_list):
        ...
        y_vals = [feat_data.loc[r, 'Mean LOC'] if r in feat_data.index else np.nan
                  for r in release_order]
...
def plot_normalized(df_sub, features_list, title_suffix, filename):
    """
    Plota Mean LOC Normalizado (0-100) ao longo das releases.
    """
    n = len(features_list)
    colors = _get_palette(n)
    ...
    for i, feat in enumerate(features_list):
        feat_data = df_sub[df_sub['feature'] == feat].set_index('release')
        y_vals = [feat_data.loc[r, 'Normalized Mean LOC'] if r in feat_data.index else np.nan
                  for r in release_order]
...
plot_mean_loc(df_metrics, sorted(feat_names),
              '(All Features)', 'mean_loc_all_features.pdf')

# Todas as features -- normalizado
plot_normalized(df_metrics, sorted(feat_names),
                '(All Features)', 'mean_loc_all_features_normalized.pdf')

# Features grandes -- absoluto e normalizado
if large_features:
    df_large = df_metrics[df_metrics['feature'].isin(large_features)]
    plot_mean_loc(df_large, large_features,
                  '(Large Features)', 'mean_loc_large_features.pdf')
    plot_normalized(df_large, large_features,
                    '(Large Features)', 'mean_loc_large_features_normalized.pdf')

# Features pequenas -- absoluto e normalizado
if small_features:
    df_small = df_metrics[df_metrics['feature'].isin(small_features)]
    plot_mean_loc(df_small, small_features,
                  '(Small Features)', 'mean_loc_small_features.pdf')
    plot_normalized(df_small, small_features,
                    '(Small Features)', 'mean_loc_small_features_normalized.pdf')

# Exportar dados processados para CSV
csv_out = os.path.join(out_dir, 'mean_loc_per_method_per_feature.csv')
